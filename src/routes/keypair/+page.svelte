<script lang="ts">
	...
	import {
		...
		generate, stop, importToWallets, exportWalletJson
...
<div class="flex flex-col">
	...
	<div class="form-row">
		<button onclick={importToWallets} disabled={!s.result} class="form-action-left !text-success {s.result ? 'marching-border' : ''}">IMPORT</button>
		<span class="flex-1 px-2 py-1 {s.importStatus ? '' : 'opacity-40'}">
			{#if s.importStatus}
				<span class={s.importStatus.type === 'error' ? 'text-error' : 'text-success'}>{s.importStatus.message}</span>
			{/if}
		</span>
		<button onclick={exportWalletJson} disabled={!s.result} class="form-action !text-success {s.result ? 'marching-border' : ''}">EXPORT</button>
	</div>
</div>
